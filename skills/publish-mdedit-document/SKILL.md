...
4. After confirmation, generate one globally unique UUID `commandId` and call `publish_article` with that command ID and `confirmPublic: true`. Reuse the exact command ID if the call must be retried. The tool atomically defaults a new link to Live while preserving an existing link's mode, so do not add a status preflight. Include a custom slug or SEO metadata only when the user supplied or approved it.
...
6. Before disabling a public link, call `get_publish_status` to identify the current link and ask for explicit confirmation. After confirmation, generate one globally unique UUID `commandId`, call `unpublish_article` with that command ID and `confirmUnpublish: true`, and reuse the same command ID for any retry. Then refresh the publish component with `render_article`.
