...
3. Generate one globally unique UUID `commandId` for this save, then call `create_article` with that same command ID, the natural workspace selector above, title, complete Markdown content, and `collaborative: false` unless the user explicitly asks for live collaboration. Reuse the exact command ID if the call must be retried.
...
5. Call `render_article` with `focus: preview` so UI-capable hosts show the saved document. Report the saved title in one short sentence and use the exact returned editor URL when a link is useful. State that the document remains private.
